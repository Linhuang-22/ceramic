...
function shareTSina(title,rLink,picUrl) {  
    ...
    var pic = "http://123.57.24.32" + ( picUrl || "/GLF/resources/img/GL-LOGO.jpg" );
...
$(function () { 
    ...
    if ($('footer .icon-wechat').parent('a').attr('data-toggle') != "modal") {
        $('footer .icon-wechat').parent('a').attr({
             'data-content': "<img src='img/weichat-qocde.jpg'>",
    ...
    //询问资料处微信分享代码
    if ($('.index-share .icon-wechat').parent('a').attr('data-toggle') != "modal") {
        $('.index-share .icon-wechat').parent('a').attr({
             'data-content': "<img src='img/weichat-qocde.jpg'>",
             'data-original-title': '<h5>微信二维码</h5>',
    ...
    //询问资料处微博链接添加
    $('.index-share .icon-weibo').parent('a').attr({
        "href": 'http://www.weibo.com/GUOLINOZONE',
        "target": '_blank'
    });
});
...
!function() {
    ...
    if (bIsIpad || bIsIphoneOs || bIsMidp || bIsUc7 || bIsUc || bIsAndroid || bIsCE || bIsWM) {
         $('.index-share .icon-wechat').parent('a').attr({//询问资料微信
             'data-toggle': 'modal',
